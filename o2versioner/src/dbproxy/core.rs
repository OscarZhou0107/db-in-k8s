...
impl Repository {
    ...
    pub async fn execute_read(&mut self) -> QueryResult {
        let _ = self.conn.query_iter("INSERT INTO cats (name, owner, birth) VALUES ('haha2', 'haha3', CURDATE())").await.unwrap();
...
fn test_helper_get_query_result_version_release() -> QueryResult{
    ...
    QueryResult {result : " ".to_string(), version_release : true, contained_newer_versions : mock_table_vs}
}
...
fn test_helper_get_query_result_non_release() -> QueryResult{
    ...
    QueryResult {result : " ".to_string(), version_release : false, contained_newer_versions : mock_table_vs}
}
...
fn voilate_dbversion_should_return_true() {
    ...
    let versions = vec![
        TableVN {table : "table1".to_string(), vn : 0, op : OperationType::R},
        TableVN {table : "table2".to_string(), vn : 1, op : OperationType::R},
        ];
